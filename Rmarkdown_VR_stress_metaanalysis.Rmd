...
gs4_deauth()
VR_study_sprdsht <- read_sheet("https://docs.google.com/spreadsheets/d/113KZYiODuhNoohl2VNSJcs1sUsDmpp_5mnv5egAPHW4/edit?usp=sharing", range = "Outcomes - Consensus!A2:DA44")
...
res2 <- rma(yi, vi, method="FE", data=dat, slab=paste(VR_study_sprdsht$Authors, VR_study_sprdsht$Year, sep=", "))
res <- rma(yi, vi, data=dat, slab=paste(VR_study_sprdsht$Authors, VR_study_sprdsht$Year, sep=", "))
mes=paste("Standardized Mean Change in EDA (", greek$mu,"S)")
funnel(res, xlab=mes)
```
...
wi <- 1/sqrt(dat$vi)
size <- 0.5 + 3 * (wi - min(wi, na.rm = TRUE))/(max(wi, na.rm = TRUE) - min(wi, na.rm = TRUE))
plot(sex_male_dum, dat$yi, type="o", pch = 19, xlab = "Group Sex Composition", ylab = "Change in SBP", xaxt="n", bty = "l", main="SMC of SBP versus Group Sex Composition")
axis(side=1, at=1:2, labels=c("all male","male+female, or all female"))
...
wi <- 1/sqrt(dat$vi)
size <- 0.5 + 3 * (wi - min(wi, na.rm = TRUE))/(max(wi, na.rm = TRUE) - min(wi, na.rm = TRUE))
plot(VR_medium, dat$yi, type="o", pch = 19, xlab = "VR medium", ylab = "Change in SBP", xaxt="n", bty = "l", main="SMC of SBP versus VR medium")
axis(side=1, at=1:2, labels=c("HMD","CAVE"))
...
wi <- 1/sqrt(dat$vi)
size <- 0.5 + 3 * (wi - min(wi, na.rm = TRUE))/(max(wi, na.rm = TRUE) - min(wi, na.rm = TRUE))
plot(VR_medium, dat$yi, type="o", pch = 19, xlab = "VR medium", ylab = "Change in DBP", xaxt="n", bty = "l", main="SMC of DBP versus VR medium")
axis(side=1, at=1:2, labels=c("HMD","CAVE"))
