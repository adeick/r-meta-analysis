...
gs4_deauth()
VR_study_sprdsht <- read_sheet("https://docs.google.com/spreadsheets/d/113KZYiODuhNoohl2VNSJcs1sUsDmpp_5mnv5egAPHW4/edit?usp=sharing", range = "Outcomes - Consensus!A2:DD60")
#nullToNA <- function(x) {
#    x[sapply(x, is.null)] <- NA
#    return(x)
#}
#nullToNA(VR_study_sprdsht)
...
if (value >= 0.001 && value < 0.1) {value <- round(value,3)}
else if (value >= 0.1 && value < 10) {value <- round(value,2)}
else if (value >= 10 && value < 100) {value <- round(value,1)}
else if (value >= 100) {value <- round(value,0)}
...
cort_m2i <- rounded_value(cort_m2i)
cort_m1i <- rounded_value(cort_m1i)
cort_sd2i <- rounded_value(cort_sd2i)
cort_sd1i <- rounded_value(cort_sd1i)
cort_n2i <- rounded_value(cort_n2i)
cort_n1i <- rounded_value(cort_n1i)
...
text(c(-9.5,-8,-6,-4.5, -2.5), 16, c("M", "SD", "M", "SD", "N")) # value 16 denotes the height,posy
text(c(-8.75,-5.25),     17, c("Baseline", "Peak")) # value three denotes the height,posy
par(op) # resets the plotting parameters.
#addpoly(res2, row=-1.7, cex=0.75, mlab="") #adds the FE statistics at the bottom
...
cat('Baseline Traditional vs During Video/VR')
...
HR_m2i <- rounded_value(HR_m2i)
HR_m1i <- rounded_value(HR_m1i)
HR_sd2i <- rounded_value(HR_sd2i)
HR_sd1i <- rounded_value(HR_sd1i)
HR_n2i <- rounded_value(HR_n2i)
HR_n1i <- rounded_value(HR_n1i)
...
funnel(res, xlab="Standardized Mean Change in Heart Rate (BPM)", legend=TRUE)
...
forest(res,xlim=c(-20,10), 
       ...
       ilab.xpos=c(-11.5,-10,-8,-6.5, -4.5), cex=.75,
       header="Author(s) and Year",
       mlab=" ")
op <- par(cex=.75, font=2)
text(c(-11.5,-10,-8,-6.5, -4.5), 45, c("M", "SD", "M", "SD", "N")) #6 denotes the height,posy
text(c(-10.75,-7.25),     46, c("Baseline", "Peak"))
par(op) # resets the plotting parameters.
...
taf <- trimfill(res) #trim-and-fill analysis
funnel(res, xlab=mes, legend=TRUE)
funnel(taf, xlab=mes, legend=TRUE)
```
...
forest(res,xlim=c(-20,6), 
       ...
       mlab=" ")
op <- par(cex=.75, font=2)
text(c(-9.5,-8,-6,-4.5, -2.5), 15, c("M", "SD", "M", "SD", "N")) #6 denotes the height,posy
text(c(-8.75,-5.25),     16, c("Baseline", "Peak"))
par(op) # resets the plotting parameters.
...
forest(res,xlim=c(-20,6), 
       ...
       mlab=" ")
op <- par(cex=.75, font=2)
text(c(-9.5,-8,-6,-4.5, -2.5), 8, c("M", "SD", "M", "SD", "N")) #6 denotes the height,posy
text(c(-8.75,-5.25),     8.5, c("Baseline", "Peak"))
par(op) # resets the plotting parameters.
...
taf <- trimfill(res) #trim-and-fill analysis
funnel(res, xlab=mes, legend=TRUE)
funnel(taf, xlab=mes, legend=TRUE) #NOTE: TAF looks weird because the Egger test say NO asymmetry, therefore TAF is not needed.
```
...
op <- par(cex=.75, font=2)
text(c(-9.5,-8,-6,-4.5, -2.5), 8, c("M", "SD", "M", "SD", "N")) #6 denotes the height,posy
text(c(-8.75,-5.25),     9, c("Baseline", "Peak"))
par(op) # resets the plotting parameters.
...
forest(res,xlim=c(-20,6), 
       ...
       mlab=" ")
op <- par(cex=.75, font=2)
text(c(-12.5,-11,-9,-7.5, -5.5), 12, c("M", "SD", "M", "SD", "N")) #6 denotes the height,posy
text(c(-11.75,-8.25),     13, c("Baseline", "Peak"))
par(op) # resets the plotting parameters.
...
text(c(-8.75,-5.25),     6.5, c("Baseline", "Peak"))
par(op) # resets the plotting parameters.
