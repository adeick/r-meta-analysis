...
forest(res,xlim=c(-20,10), 
       ...
       ilab.xpos=c(-9.5,-8,-6,-4.5, -2), cex=.75,
       header="Author(s) and Year",
       mlab=" ")
op <- par(cex=.75, font=2)
text(c(-9.5,-8,-6,-4.5,-2), 24, c("M", "SD", "M", "SD", "N")) #6 denotes the height,posy
...
forest(res,xlim=c(-20,10), 
       ...
       mlab=" ")
op <- par(cex=.75, font=2)
text(c(-9.5,-8,-6,-4.5, -2), 11.5, c("M", "SD", "M", "SD", "N")) #6 denotes the height,posy
...
forest(res,xlim=c(-20,10), 
       ...
       mlab=" ")
op <- par(cex=.75, font=2)
text(c(-9.5,-8,-6,-4.5, -2), 4.2, c("M", "SD", "M", "SD", "N")) #6 denotes the height,posy
text(c(-8.75,-5.25),     12.2, c("Baseline", "Peak"))
par(op) # resets the plotting parameters.

#bottom labels
addpoly(res2, row=-1.5, cex=0.75, mlab="")
text(-20, -0.5, pos=4, cex=0.75, bquote(paste("RE Model (Q = ",
     .(formatC(res$QE, digits=2, format="f")), ", df = ", .(res$k - res$p),
     ", p = ", .(formatC(res$QEp, digits=2, format="f")), "; ", I^2, " = ",
     .(formatC(res$I2, digits=1, format="f")), "%)")))
text(-20, -1.5, pos=4, cex=0.75, bquote(paste("FE Model (Q = ",
...
XX_m2i=as.numeric(unlist(select(VR_study_sprdsht,c(66))))#baseline  #select the columns you want, then convert from list to numeric array
XX_m1i=as.numeric(unlist(select(VR_study_sprdsht,c(71)))) #peak 
XX_sd2i=as.numeric(unlist(select(VR_study_sprdsht,c(67)) )) 
XX_sd1i=as.numeric(unlist(select(VR_study_sprdsht,c(72)))) 
XX_ni=as.numeric(unlist(select(VR_study_sprdsht,c(75)))) 

dat <-escalc(measure="SMCC", ni=XX_ni, m1i=XX_m1i, m2i=XX_m2i, sd1i=XX_sd1i, sd2i=XX_sd2i, ri=rep.int(0.5, 27)) #SMCC= Standardized mean change 
...
forest(res,header=TRUE, cex=.8)
forest(res,xlim=c(-20,10), 
       ilab=cbind(XX_m2i, XX_sd2i, XX_m1i, XX_sd1i, XX_ni),
       ilab.xpos=c(-9.5,-8,-6,-4.5, -2), cex=.75,
       header="Author(s) and Year",
       mlab=" ")
op <- par(cex=.75, font=2)
